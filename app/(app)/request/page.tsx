...
export default function RequestPage() {
  ...
  const loadSeekerRequests = React.useCallback(async () => {
    ...
    try {
      ...
      const resPending = await fetch(
        "/api/requests?role=seeker&status=pending",
      );
      const dataPending = await resPending.json();
      setPendingRequest(
        dataPending.success && dataPending.data?.length > 0
          ? dataPending.data[0]
          : null,
      );
  ...
  const handleRequestCreated = (req: any) => {
                ...
                {seekerActiveRequests.length === 0 ? (
                  ...
                  <div className="space-y-2.5">
                    {seekerActiveRequests.map((req) => (
                      ...
                      >
                        <div>
                          <h5 className="text-sm font-bold text-white truncate pr-6">
                            {req.title}
                          </h5>
          ...
          {isLoadingHelper ? (
            ...
            <div className="bg-[#131B2E]/20 p-12 rounded-lg border border-dashed border-white/10 text-center max-w-xl mx-auto space-y-3 py-16">
              ...
              <p className="text-xs leading-normal max-w-xs mx-auto text-slate-500">
                We couldn&apos;t find any pending help requests near your
                location right now. Check back later!
              </p>
            </div>
          ) : (
            (() => {
              const exactMatch = communityRequests.filter(
                (r: any) => r._skillRank === 1,
              );
              const related = communityRequests.filter(
                (r: any) => r._skillRank === 2,
              );
              const others = communityRequests.filter(
                (r: any) => r._skillRank === 3 || !r._skillRank,
              );

              const renderCard = (req: any) => {
                const colorClass =
                  SKILL_COLORS[req.category] ||
                  ...
                  >
                    <div className="flex-1 min-w-0 space-y-2">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`text-[10px] font-bold border rounded-sm px-2 py-0.5 capitalize ${colorClass}`}
                        >
                          {req.category}
                        </span>
                        <span
                    ...
                    <Button
                      ...
                      className={cn(
                        "font-semibold text-xs h-9 px-5 rounded-md flex items-center justify-center shrink-0 w-full md:w-auto transition-colors",
                        isUrgent
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-xs font-bold uppercase tracking-wider text-emerald-400">
                          🎯 Your Skills — Best Match ({exactMatch.length})
                        </h4>
                      </div>
                      <div className="space-y-3">
                        {exactMatch.map(renderCard)}
                      </div>
                    </div>
                  )}
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-xs font-bold uppercase tracking-wider text-indigo-400">
                          📍 Near You — Related Skills ({related.length})
                        </h4>
                      </div>
