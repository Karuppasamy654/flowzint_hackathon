...
import { ImpactDashboard } from '@/components/request/ImpactDashboard';
...
export default function RequestPage() {
  ...
  const loadSeekerRequests = React.useCallback(async () => {
    ...
    try {
      ...
      if (dataPending.success && dataPending.data?.length > 0) {
        setPendingRequest(dataPending.data[0]);
      } else {
      ...
      if (dataActive.success) {
        setSeekerActiveRequests(dataActive.data || []);
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      {/* Top Banner section */}
      <div className="text-left py-2 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-display font-semibold text-white leading-tight">
            Community Dashboard
          </h1>
          <p className="text-sm text-slate-400">
            Request assistance from neighbors or share your skills to help others.
          </p>
        </div>
      </div>

      {/* Live impact stat widgets */}
      <ImpactDashboard />
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full text-left">
            ...
            <div className="space-y-4">
              <div className="bg-[#131B2E]/40 border border-white/10 p-4 rounded-lg flex items-center justify-between shadow-2xl backdrop-blur-md">
                ...
                <span className="text-xs text-slate-400">
                  Posted {formatDistanceToNow(new Date(pendingRequest.createdAt), { addSuffix: true })}
                </span>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start animate-in fade-in duration-300">
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">
                  Active Help Sessions ({seekerActiveRequests.length})
                </h4>

                {seekerActiveRequests.length === 0 ? (
                  <div className="bg-[#131B2E]/20 p-6 rounded-lg border border-dashed border-white/10 text-center space-y-2 text-slate-500">
                    <MessageSquare className="h-8 w-8 text-slate-600 mx-auto" />
                    <p className="text-xs font-semibold text-slate-300">No active help sessions</p>
                    <p className="text-[11px] leading-normal text-slate-500">
                      When someone accepts your request, they will appear here to coordinate via chat.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2.5">
                    {seekerActiveRequests.map((req) => (
                      <div
                        ...
                        className="bg-[#131B2E]/40 border border-white/10 p-4 rounded-lg hover:border-indigo-500/30 hover:shadow-[0_0_15px_rgba(99,102,241,0.15)] transition-all cursor-pointer text-left space-y-2.5 relative backdrop-blur-md"
                      >
                        <div>
                        ...
                        <div className="flex items-center justify-between text-xs pt-1 border-t border-white/5">
                          <span className="text-slate-400 font-medium truncate">
                            Helper: {req.acceptedHelper?.name || 'Assigned'}
                          </span>
                          <span className="text-indigo-400 font-bold flex items-center gap-0.5 text-[11px] shrink-0 group-hover:translate-x-0.5 transition-transform">
                            Open Chat &rarr;
                          </span>
                        </div>
                      </div>
        ...
        <TabsContent value="offer-help" className="space-y-4">
          {isLoadingHelper ? (
            ...
            <div className="bg-[#131B2E]/20 p-12 rounded-lg border border-dashed border-white/10 text-center max-w-xl mx-auto space-y-3 text-slate-500 py-16">
              <AlertCircle className="h-10 w-10 text-slate-600 mx-auto" />
              <p className="text-sm font-semibold text-slate-300">No pending requests nearby</p>
              <p className="text-xs leading-normal max-w-xs mx-auto text-slate-500">
                We couldn&apos;t find any pending help requests in the community right now. Check back later!
              </p>
            </div>
          ) : (
            <div className="space-y-3 max-w-3xl animate-in fade-in duration-300">
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 pl-1">
                Pending Requests ({communityRequests.length})
              </h4>
                ...
                {communityRequests.map((req) => {
                  ...
                  return (
                    <div
                      key={req._id}
                      className={cn(
                        "bg-[#131B2E]/45 border p-5 rounded-lg shadow-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-left backdrop-blur-md transition-all duration-300",
                        isUrgent
                          ? "border-red-500/40 shadow-[0_0_20px_rgba(239,68,68,0.12)] hover:border-red-500/60"
                          : "border-white/10 hover:border-indigo-500/20 shadow-lg"
                      )}
                    >
                      <div className="flex-1 min-w-0 space-y-2">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className={`text-[10px] font-bold border rounded-sm px-2 py-0.5 capitalize ${colorClass}`}>
                            {req.category}
                          </span>
                          <span className={cn(
                            "text-[10px] font-bold border rounded-sm px-2 py-0.5 capitalize",
                            isUrgent 
                      ...
                      <Button
                        ...
                        className={cn(
                          "font-semibold text-xs h-9 px-5 rounded-md shadow-lg flex items-center justify-center shrink-0 w-full md:w-auto transition-colors",
                          isUrgent
